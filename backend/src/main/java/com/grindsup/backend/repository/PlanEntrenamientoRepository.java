...
public interface PlanEntrenamientoRepository extends JpaRepository<PlanEntrenamiento, Long> {
        ...
        // 3. Consulta para obtener la entidad PlanEntrenamiento COMPLETA por Entrenador
        // ID (Mantenemos esta y eliminamos la duplicada/ambigua)
        @Query("""
                        select p
        ...
        // Selecciona campos para el DTO. Accede al ID del plan como 'p.id_plan' y al
        // entrenador como 'p.entrenador.idEntrenador'.
        @Query("""
        ...
        @Query("SELECT p FROM PlanEntrenamiento p JOIN p.estado e " +
                        "WHERE p.fecha_fin = :hoy " +
                        "AND e.nombre <> 'CALIFICADO'")
        List<PlanEntrenamiento> findPlanesTerminandoHoyYPendientesDeCalificar(
