...
// 🚀 ¡CAMBIO 1: IMPORTAMOS TU AXIOS CONFIGURADO!
...
export function getEntrenadorName(user) {
  // ... (tu lógica de getEntrenadorName estaba bien)
  if (!user) return null;
  const ent = user?.entrenador || user?.usuario || user;
  const nombre = ent?.nombre ?? user?.nombre ?? ent?.firstName ?? user?.firstName ?? null;
  const apellido = ent?.apellido ?? user?.apellido ?? ent?.lastName ?? user?.lastName ?? null;
  if (nombre || apellido) {
...
  // 3) último recurso: preguntar al backend (¡AHORA CON AXIOS!)
  try {
    if (!user?.id_usuario) return null;

    // 🚀 ¡CAMBIO 2: USAMOS "api" (TU AXIOS) EN LUGAR DE "fetch"!
    // a) /entrenadores?usuarioId=...
    try {
      const { data } = await api.get(`/api/entrenadores?usuarioId=${user.id_usuario}`);
      const id = Array.isArray(data) && data.length ? data[0]?.id_entrenador ?? data[0]?.id : null;
      if (id) {
        localStorage.setItem("entrenadorId", String(id));
        return Number(id);
      }
    } catch (q1Error) {
      console.warn("ensureEntrenadorId (query 1) falló", q1Error);
    }
    
    // b) /entrenadores/by-usuario/{id}
    try {
      const { data } = await api.get(`/api/entrenadores/by-usuario/${user.id_usuario}`);
      ...
      if (id) {
       ...
       console.warn("ensureEntrenadorId (query 2) falló", q2Error);
    }
  } catch (err) {
...
  if (userName) {
    if (cachedName !== userName) localStorage.setItem("entrenadorName", userName);
    return { id, displayName: userName };
  }
  if (cachedName) return { id, displayName: cachedName };

  // 🚀 ¡CAMBIO 3: USAMOS "api" (TU AXIOS) EN LUGAR DE "fetch"!
  if (id) {
    try {
      const { data: ent } = await api.get(`/api/entrenadores/${id}`);
      if (ent) {
        const nombre = [ent?.nombre, ent?.apellido].filter(Boolean).join(" ").trim();
        const displayName = nombre || ent?.username || (ent?.email ? ent.email.split("@")[0] : null) || null;
        if (displayName) {
...
  useEffect(() => {
    // 🚀 ¡CAMBIO 4: USAMOS "gs_token" (EL NOMBRE CORRECTO)!
